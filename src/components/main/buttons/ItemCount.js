import { useEffect, useState } from "react";

const ItemCount = (props) => {
  const [products, setProducts] = useState(props.initial);
  useEffect(() => {
    setProducts(props.initial);
  },[]);
  const increment = () => {
    if(products < props.stock) setProducts(products + 1);
  }
  const decrement = () => {
    if(products > props.initial + 1) setProducts(products - 1);
  }
  return (
    <>
      <div className="card__buttons">
        <button onClick={decrement} className="btn btn__count">-</button>
        <p className="card__products">Productos: {products} / {props.stock}</p>
        <button onClick={increment} className="btn btn__count">+</button>
      </div>
      {
        props.stock && products ?
        <button className="btn btn__addToCar" id={`buy${props.id}`} onClick={() => props.onAdd(products)}>Agregar al carrito</button> :
        <button className="btn btn__addToCar btn__addToCar--inactive" id={`buy${props.id}`}>Agregar al carrito</button>
      }
    </>
  )};
export default ItemCount;